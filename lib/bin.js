...
var addRepositories = function (response) {
 ...
 */
function WpkgBin (response, callback) {
  const self = this;
  ...
  this._pacmanConfig = require ('xcraft-core-etc') (null, response).load ('xcraft-contrib-pacman');
   ...
   */
  var run = function (args, lastArg, tmp, response, callback, callbackStdout) {
    ...
    response.log.info ('begin command ' + cmdName);
    ...
    response.log.verb ('%s %s', bin, args.join (' '));

    xProcess.spawn (bin, args, {}, function (err, code) {
      /* When the call is terminated. */
      response.log.info ('end command ' + cmdName + ' with rc ' + code);
  ...
  this._run = function (args, lastArg, callbackStdout) {
    xSubst.wrap (_tmp, response, function (err, dest, callback) {
      run (args, lastArg, dest, response, callback, callbackStdout);
    }, _callback);
  };
}
...
WpkgBin.prototype.build = function (repositoryPath, packagePath, arch) {
  ...
  args = args.concat (addRepositories (this._response));
  args.push ('--build');
...
WpkgBin.prototype.listIndexPackages = function (repositoryPath, arch, filters, listOut) {
  ...
  this._run (args, path.join (repositoryPath, this._pacmanConfig.pkgIndex), function (line) {
