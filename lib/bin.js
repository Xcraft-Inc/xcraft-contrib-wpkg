...
class WpkgBin {
  ...
  _run (args, lastArg, callbackStdout, callback) {
  ...
  build (repositoryPath, packagePath, arch, callback) {
    let args = [];
    ...
    this._run (args, packagePath, null, callback);
  }

  buildSrc (repositoryPath, callback) {
    const args = [
      '--verbose',
      '--output-repository-dir', repositoryPath || this._xcraftConfig.pkgDebRoot,
      '--cmake-generator', this._cmake,
    ...
    this._run (args, null, null, callback);
  }

  createIndex (repositoryPath, indexName, callback) {
    const args = [
      '--verbose',
    ...
    this._run (args, path.join (repositoryPath, indexName), null, callback);
  }

  install (packagePath, arch, reinstall, callback) {
    let args = [
      '--verbose',
      '--force-file-info',
  ...
  isInstalled (packageName, arch, callback) {
    ...
    this._run (args, packageName, null, callback);
  }
  ...
  createAdmindir (controlFile, arch, callback) {
    const args = [
      '--verbose',
      '--root', path.join (this._xcraftConfig.pkgTargetRoot, arch),
      '--create-admindir'
    ...
    this._run (args, controlFile, null, callback);
  }

  addSources (source, arch, callback) {
    const args = [
      '--verbose',
      '--root', path.join (this._xcraftConfig.pkgTargetRoot, arch),
      '--add-sources'
    ...
    this._run (args, source, null, callback);
  }

  listSources (arch, listOut, callback) {
    const args = [
      '--root', path.join (this._xcraftConfig.pkgTargetRoot, arch),
      '--list-sources'
    ];
  ...
  listFiles (packageName, arch, listOut, callback) {
    const args = [
      '--root', path.join (this._xcraftConfig.pkgTargetRoot, arch),
      '--listfiles'
    ];
  ...
  listIndexPackages (repositoryPath, arch, filters, listOut, callback) {
