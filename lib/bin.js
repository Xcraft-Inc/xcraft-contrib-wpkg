...
class WpkgBin {
  ...
  * createIndex (repositoryPath, indexName, next) {
    ...
    return yield this._run (args, path.join (repositoryPath, indexName), null, next);
  }
  ...
  * createAdmindir (controlFile, arch, next) {
    ...
    return yield this._run (args, controlFile, null, next);
  }
  ...
  * addSources (source, arch, next) {
    ...
    return yield this._run (args, source, null, next);
  }
  ...
  * listSources (arch, listOut, next) {
    ...
    return yield this._run (args, null, (line) => {
      if (!line.trim ().length) {
        return;
      }
  ...
  * listFiles (packageName, arch, listOut, next) {
    ...
    return yield this._run (args, packageName, (line) => {
      if (!line.trim ().length) {
        return;
      }
  ...
  * isV1Greater (v1, v2, next) {
    ...
    const code = yield this._run (args, null, null, next);
    return code === 0;
  }
  ...
  * listIndexPackages (repositoryPath, arch, filters, listOut, next) {
    ...
    const res = yield this._run (args, path.join (repositoryPath, this._pacmanConfig.pkgIndex), (line) => {
