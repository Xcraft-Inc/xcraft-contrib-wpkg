...
class WpkgBin {
  ...
  constructor (resp) {
    ...
    this._resp = resp;
    this._xcraftConfig = require ('xcraft-core-etc') (null, resp).load (
      'xcraft'
    );
  ...
  *build (repositoryPath, packagePath, arch, next) {
    ...
    args = args.concat ([
      ...
      repositoryPath || this._xcraftConfig.pkgDebRoot,
      '--install-prefix',
      '/usr',
      '--compressor',
      'gz',
  ...
  *install (packagePath, arch, reinstall, next) {
    let args = [
      ...
      '--force-file-info',
      '--root',
      path.join (this._xcraftConfig.pkgTargetRoot, arch),
    ];
